...
const ReviewSection: React.FC<ReviewSectionProps> = ({ productId }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await axios.get(`${serverUrl}/api/reviews/product/${productId}`);
        setReviews(res.data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await axios.get(`${serverUrl}/api/users/${sessionStorage.getItem("mongoUserId")}`);
